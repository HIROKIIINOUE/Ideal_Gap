...
export const sanitizeFocusMusicExtension = (storagePath: string) => {
...
export const getFocusMusicFileName = (
  track: Pick<FocusMusicTrack, "id" | "storagePath">,
) => `${track.id}${sanitizeFocusMusicExtension(track.storagePath)}`;

export const getFocusMusicLocalUri = (fileName: string) =>
  `${getFocusMusicDirectory()}${fileName}`;

export const getFocusMusicLocalUriForTrack = (
  track: Pick<FocusMusicTrack, "id" | "storagePath">,
) => getFocusMusicLocalUri(getFocusMusicFileName(track));
...
export const downloadTrackFile = async (
  ...
  const filePath = `${dir}${getFocusMusicFileName(track)}`;
...
export const getFocusMusicFileInfo = async (fileName: string) => {
  const localPath = getFocusMusicLocalUri(fileName);
  try {
    const info = await FileSystem.getInfoAsync(localPath);
    return {
      exists: info.exists === true && info.isDirectory !== true,
      localPath,
    };
  } catch {
    return {
...
export const deleteTrackFile = async (fileName: string): Promise<void> => {
  await FileSystem.deleteAsync(getFocusMusicLocalUri(fileName), {
    idempotent: true,
  });
};
